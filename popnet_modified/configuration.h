//参考
//https://www.cnblogs.com/x2i0e19linux/p/11812231.html
...
//changed at 2020-5-8
extern const time_type TIME_0;

/*******************************************************/
class configuration {
	friend ostream & operator<<(ostream &, const configuration & cf);
	private:
		long ary_number_; //k-ary 每个维度上的路由器个数，比如一个二维的9*9网络的此参数就是9
		long cube_number_;  //n-cube 维度
		...
		time_type report_period=REPORT_PERIOD_;
		//changed at 2022-4-20
		std::string reconfigurationFilePath;
	...
	public:
		...
		time_type getReportPeriod()const;
		//changed at 2022-4-20
		const std::string&getReconfigurationFilePath()const;
