...
constexpr time_type LOCAL_INPUT_TIME_0=numeric_limits<time_type>::infinity();
...
std::unordered_set<flit_template::TPacketId>sim_router_template::abandonedPacketIds;
...
void sim_router_template::receive_packet()
{
													 ...
													 event_time + S_ELPS_))
	{
		...
		inject_packet(packet_counter_, /* sor_addr_t */p.sourceAddress,
					  /* des_addr_t */p.destinationAddress, local_input_time_, /* pack_size_t */p.packetSize,p.id);
		packet_counter_++;
	...
	//拥堵时输出信息
#ifdef FILTERING
	if(input_module_.ibuff_full()){
		logfile<<"Jam at time "<<mess_queue::m_pointer().current_time()<<" in router (";
		bool b=false;
		for(auto&a:address_){
			if(b)logfile<<", ";
			else b=true;
...
void sim_router_template::inject_packet(long a, add_type &b, add_type &c,
										time_type d, long e,SPacket::TId packet_id)
{
...
{
	//丢包
	if(configuration::ap().getPacketLoss()){
		if(input_module_.inputBufSize(a,b)>=buffer_size_){
			abandonedPacketIds.insert(c.getPacketId());
		}
		if(abandonedPacketIds.count(c.getPacketId())>0){
			return;
		}
	}
...
void sim_router_template::router_sim_pwr(time_type routingPeriod)
{
	//changed at 2022-4-12
	/* routingTime+=PIPE_DELAY_;
	if(routingTime+S_ELPS_<pipelineStageDelay())return;
	routingTime=0; */
	if(routingPeriod!=pipelineStageDelay())return;
...
time_type sim_router_template::pipelineStageDelay()const
{
	return PIPE_DELAY_;
}
...
void CGraphTopo::tableRoutingAlg(const add_type&dst,const add_type&src,long s_ph,long s_vc,
	TIntMatrix&routing_table,unique_ptr<std::unordered_map<TAddressNumber,TAddressNumber>[]>&next_hop_port_map)
{
	TAddressNumber nextAdd=routing_table[getAddressNumber()][dst.front()];
	...
	//尝试全部虚通道
	addRoutingForDifferentVC(input_module_,s_ph,s_vc,port,configuration::ap().virtual_channel_number());
...
time_type CGraphTopo::tableWireDelay(long port,unique_ptr<vector<CGraphTopo::SLinkInfo>[]>&port_map)
{
	return port_map[getAddressNumber()][port].linkDelay;
}
time_type CGraphTopo::getWireDelay(long port)
{
	return tableWireDelay(port,portMap);
}
void CGraphTopo::getNeighbour(add_type&neighbourAddress,long port,unique_ptr<vector<CGraphTopo::SLinkInfo>[]>&port_map)
{
	neighbourAddress.clear();
	neighbourAddress.push_back(port_map[getAddressNumber()][port].neighbour);
}
void CGraphTopo::getNextAddress(add_type&nextAddress,long port)
{
	getNeighbour(nextAddress,port,portMap);
}
long CGraphTopo::getNeighbourPort(long port,unique_ptr<vector<CGraphTopo::SLinkInfo>[]>&port_map)
{
	return port_map[getAddressNumber()][port].neighbourPort;
}
...
time_type CGraphTopo::pipelineStageDelay()const
{
	return topo0[topoVertices[getAddressNumber()]].pipelineStageDelay;
	//return PIPE_DELAY_;
}
...
unique_ptr<vertex_t[]>CGraphTopo::topoVertices;
void CGraphTopo::readTopo()
{
	Sassert(readGvGraph(configuration::ap().getTopoFilePath(),topo0));
	resetIndex(topo0);
}
void CGraphTopo::otherInit()
{
	vertexCnt=num_vertices(topo0);
}
void CGraphTopo::setPipelineDelay(graph_t&topo)
{
	auto nodes=vertices(topo);
	auto pipelineStageDelay=get(&vertex_info::pipelineStageDelay,topo);
	for_each(nodes.first,nodes.second,[&](const vertex_t&v){
		routing_periods.insert(pipelineStageDelay[v]);
	});
}
void CGraphTopo::setPortCnt(graph_t&topo)
{
...
void CGraphTopo::setTopoVertices(graph_t&topo)
{
	topoVertices=make_unique<vertex_t[]>(vertexCnt);
	auto nodes=vertices(topo);
