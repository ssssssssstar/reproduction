...
void mess_queue::setInitEvent()
{
	switch(configuration::ap().routing_alg()){
	case RECONFIGURABLE_GRAPH_TOPO:
		add_message(mess_event(TIME_0-S_ELPS_/2,RECONFIGURATION));
	case GRAPH_TOPO:
		for(auto&p:routing_periods){
			add_message(mess_event(TIME_0,ROUTER_,p));
		}
		break;
	default:
		add_message(mess_event(TIME_0, ROUTER_));
